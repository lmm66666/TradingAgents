...
`.claude/commands/` 目录下提供 Claude Code 技能文件，打开项目后自动识别，可在对话中通过斜杠命令触发股票分析流程：

| 命令 | 说明 |
|------|------|
| `/analysis_daily_b1` | 日线 B1 股票筛选：结合技术形态与财务基本面，输出分级买入建议 |
| `/analysis_weekly_b1` | 周线 B1 股票筛选：从估值、盈利、行业、成长、现金流五大维度深度分析 |

分析结果固定输出到 `./docs/analysis/` 目录。
